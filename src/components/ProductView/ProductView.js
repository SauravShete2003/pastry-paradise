...
function ProductView() {
  const [productData, setProductData] = useState([...birthdayData]);
  const [searchText, setSearchText] = useState("");

  useEffect(() => {
    if (!searchText) {
      setProductData([...birthdayData]);
      return;
    }
    const newData = productData.filter((productObj) => {

      console.log(productObj)
      return productObj.name.toLowerCase().includes(searchText);
    });
    console.log(newData)
  ...
  return (
    <>
      <div>
        ...
        <h2 className="birthday-heading">Popular Cakes</h2>
        <input 
        type="text"
